  ...
  <Transition name="fly-in">
    <div class="preference-tooltip" v-if="show">
      <template v-if="source === 'default'">
        <p>O estilo da API agora predefine para API de Composição.</p>
        <p>
          Algumas páginas contêm conteúdo diferente baseado no estilo de API escolhido. Use este interruptor para alternar entre os estilos de APIs.
        </p>
      </template>
      <template v-if="source && source.startsWith('url')">
        <p>
          Showing content for
          Mostrando conteúdo para a API de
          {{ preferComposition ? 'Composição' : 'Opções' }} porque
          {{
            source === 'url-query'
              ? 'está especificada pela consulta de URL.'
              : 'a seção escolhida como alvo está disponível apenas para esta API.'
          }}
        </p>
        <p>
          Isto é diferente da tua preferência guardada e apenas afetará a sessão da pesquisa atual.
        </p>
      </template>
      <p class="actions">
        <a href="/guide/introduction#api-styles">Saiba mais</a>
